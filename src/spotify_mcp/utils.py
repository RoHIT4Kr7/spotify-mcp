...
def normalize_redirect_uri(url: str) -> str:
    """
    Normalize redirect URI for Spotify OAuth compatibility.

    Spotify requirements:
    - No wildcard domains
    - No 0.0.0.0 (considered insecure)
    - localhost is acceptable for development
    - HTTPS required for production (ngrok provides this)
    """
    if not url:
        return url
    ...
    # Convert 0.0.0.0 to 127.0.0.1 (localhost equivalent)
    # Spotify considers 0.0.0.0 as insecure
    if parsed.netloc == "0.0.0.0" or parsed.netloc.startswith("0.0.0.0:"):
        port = ""
        if ":" in parsed.netloc:
    ...
    elif parsed.netloc == "localhost" or parsed.netloc.startswith("localhost:"):
        port = ""
        if ":" in parsed.netloc:
            port = ":" + parsed.netloc.split(":")[1]
        parsed = parsed._replace(netloc=f"127.0.0.1{port}")
...
def get_redirect_uri_for_environment() -> str:
    """
    Get the appropriate redirect URI based on the environment.

    Returns:
        - For development: http://127.0.0.1:8000/callback
        - For production with ngrok: Should be set via environment variable
    """
    import os

    # Check if running with ngrok (via environment variable)
    ngrok_url = os.environ.get("NGROK_URL")
    if ngrok_url:
        # Ensure it ends with /callback
        if not ngrok_url.endswith("/callback"):
            ngrok_url = ngrok_url.rstrip("/") + "/callback"
        return ngrok_url
